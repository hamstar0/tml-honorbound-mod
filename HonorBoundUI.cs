	...
	public class HonorBoundUI : UIState {
		...
		public override void OnInitialize() {
			...
			foreach( var kv in HonorBoundLogic.Honorifics ) {
				...
				if( !HonorBoundLogic.HonorificAllowed[ honorific ] ) { continue; }

				var option = new UICheckbox( honorific, String.Join("\n", kv.Value) );
				option.Top.Set( top, 0f );
				option.SetText( "    "+honorific, 0.8f, false );
				option.TextColor = Color.Gray;

				option.OnSelectedChanged += delegate () {
				...
				this.Options[honorific] = option;
				this.MainPanel.Append( option );
			}
		...
		private MouseEvent MouseOver( UIMouseEvent uIMouseEvent, object evt, UIElement uIElement, object listeningElement ) {
		...
		public void RefreshAllowedOptions() {
			if( !this.HasBeenSetup ) { return; }

			bool has_changed = false;

			foreach( var kv in HonorBoundLogic.HonorificAllowed ) {
				string honorific = kv.Key;
